...
\begin{frame}
  ...
  \begin{bibliolist}{00}
    ...
    \newblock 2021. \url{https://www.bilibili.com/video/BV1L44y1676h}.
    \link{http://www.alanshawn.com/latex3-tutorial/}
  ...
  \note[item]{由于第一篇文章的贡献者比较多，所以只以出现位置的所属作为作者。}
  \note[item]{项子越的主页 \link{http://www.alanshawn.com/latex3-tutorial/} 有全面的英文版资料，
  甚至还有使用 \LaTeX3 刷 LeetCode 题的例子 \link{http://www.alanshawn.com/advanced-typesetting/}。}
  \note[item]{还有 \link{https://liam.page/2015/01/23/literate-programming-in-latex/} 可供参考。}
\end{frame}
...
\begin{frame}
  ...
  \note[item]<1>{以及 Word 中的多倍行距是不严谨的（详见 \pkg{zhlineskip} 宏包），
  对于不同的字体会有不同的实际倍数的行距。使用磅值定义行间距是严谨的，在严肃排版中经常使用，
  个人认为，是为了方便用户别去记那么多的磅值有了多倍行距这种说法，这个概念最好限定于日常使用中。}
...
|\only<2>{\highlightline\ttfamily\cmd{ExplSyntaxOff}}|
    ...
    \end{column}
  \end{columns}
  \note<2>[item]{\LaTeXe{} 固然不错，但是它提供的功能还是没有那么全面：比如只能是第一个参数为可选参数，搞个键值对参数需要引入宏包；
...
\begin{frame}[fragile]
  ...
  \only<1-2>{模板的大致结构是接受选项；提供文档接口；传递选项给其他模块，引用其他模块的命令。}
...
\begin{frame}[fragile]
  ...
  \note{Jupyter Notebook 也是一种文学编程，将代码生成文档的相关工具（比如 Sphinx）也属于文学编程。}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{.6\textwidth}
      ...
      \begin{description}
        ...
        \item[\ttfamily l3build ctan] 执行上述所有流程，输出部署结果至 \texttt{build/distrib}，生成 \texttt{*-ctan.zip} 压缩文件。\link{https://tug.org/tds/}
  ...
  *-ctan.zip 中可以包含 .tds.zip，可以放在 \TeX{} 发行版的 TDS (\TeX{} Directory Structure) 文件目录下，之后刷新文件名数据库。

  学完这些，你大概就可以看懂（can read）源代码，做出给自己使用的“宏包”了，甚至是一个“模板”：
