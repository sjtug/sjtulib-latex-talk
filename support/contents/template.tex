...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item<4> 保持与标准类统一的接口，可以方便用户迁移内容于其他模板。
  \end{itemize}
...
\begin{frame}[fragile]
  ...
  \only<8>{\cmd{LoadClass} 用来加载一个基础文档类。}
  \only<9>{也就是引入文档类时中括号内传入的选项。}
  ...
  \begin{columns}
    \only<3-8,10,12-14>{\hspace*{2em}}
    \only<9,11>{\hspace*{-.6\textwidth}}
    \begin{column}<3-8,10,12-14>{.6\textwidth}
...
|\highlightline<8>|\LoadClass{article}
...
|\highlightline<14>|\othercmd % provided by myother
      \end{codeblock}
    \end{column}
    \only<1-2,9,11>{\hspace*{-8em}}
    \begin{column}{.35\textwidth}
      \begin{figure}
        \centering
        \alt<4-9>{\highlight{选项}}{选项} \\
        $\downarrow$ \\
        \framebox[6em]{模板}\rlap{$\rightarrow$ \alt<10-11>{\highlight{接口}}{接口}} \\
        \only<1,3-11>{$\downarrow$ \\ 其他模块}
        \only<12-14>{$\downarrow$ \\ \highlight{其他模块}}
        \only<2>{%
      ...
      \end{figure}
    \end{column}
    \only<3-8,10,12-14>{\hspace*{-10em}}
    \begin{column}<9,11>{.5\textwidth}
      \begin{codeblock}[]{testmyart.tex}
|\highlightline<9>|\documentclass[sjtu]{myart}
\begin{document}
