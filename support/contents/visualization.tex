...
\begin{frame}[fragile,label=tikz]
  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
      ...
      \only<1>{
        可以直接在文档导言区使用 \TikZ{} 包 \link{https://mirrors.sjtug.sjtu.edu.cn/CTAN/graphics/pgf/base/doc/pgfmanual.pdf}，或者采用 \pkg{standalone} 文档类。 
      ...
      \only<4>{
        \cmd{draw} 用来描边，\pkg{edge} 用来指示这是一条线段，参数 \pkg{->} 用来说明这是一个箭头。紧随其后可以添加线段上的节点，\pkg{above},\pkg{below},\pkg{left},\pkg{right} 可以用来指示相对位置。
      }
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
      \begin{figure}
        \centering
        \includegraphics[height=0.5\textheight]{tikzedtui}
        \caption{\TikZ{}Edt 界面}
      \end{figure}
    \end{column}
    \begin{column}{0.4\textwidth}
      \begin{itemize}
        ...
        \includegraphics[width=\linewidth]{tooltip}
        \item 开始时会提示生成缩略图
...
\begin{frame}[fragile]
  \frametitle{PGFPlots}
  \onslide<1>{
    \pgfplots{} \link{https://mirrors.sjtug.sjtu.edu.cn/CTAN/graphics/pgf/contrib/pgfplots/doc/pgfplots.pdf} 是基于 \pgf{}/\TikZ{} 的宏包，用于生成高质量中等数据规模的统计图。
  }
  \begin{columns}
    \begin{column}{0.4\textwidth}
      ...
      \only<1>{
        \begin{center}
          \includegraphics[width=0.75\textwidth]{pgfplots}
        \end{center}
      }
...
\begin{frame}
  ...
  \raisebox{-\dp\strutbox}{\includegraphics[height=\baselineskip]{pgfplotsedt}} PGFPlotsEdt \link{https://logcreative.github.io/PGFPlotsEdt/?lang=cn} 是一款基于 \pgfplots{} 的统计绘图编辑器，基于 \faVuejs{} 开发。
  ...
  \begin{columns}
    ...
    \begin{column}{0.6\textwidth}
      \begin{figure}
        \centering
        \includegraphics[height=0.4\textheight]{pgfplotsedtui}
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.58\textwidth}
      \begin{itemize}
        ...
        \note{不只是 \pgfplots{}。}
        \item 推荐使用 \pkg{standalone} 文档类编译再插入。
    ...
    \begin{column}{0.42\textwidth}
      \begin{figure}
        \begin{tikzpicture}[scale=0.7,every node/.style={scale=0.7}]
          \node[draw,fill=white] (v1) at (-1.5,0) {\includegraphics[scale=0.7]{tkzeuclide}};
          \node[draw,fill=white] at (1.5,-2) {\includegraphics[scale=0.3]{circuitikz}};
          \node[draw,fill=white] at (-1.5,-3) {\includegraphics[scale=0.3]{automata}};
        \end{tikzpicture}
  ...
  2020. \url{https://www.latexstudio.net/archives/51804.html}}
  
  \note{看文档需要使用 texdoc pgf，这个笔记大略翻译了很多内容。\LaTeX{} 工作室的网站上也有很多 \TikZ{} 的 fancy 内容，感兴趣的可以参考。}

  \note{可以做的很 fancy，也可以用它很基本的功能。因为有一种职业叫科技绘图，这种图形约是符合要求的矢量图。\TikZ{} 的意义在于提供了一种 inline 绘图的方式，有助于管理工程，过早优化万恶之源，这样的方式有助于减少重新绘制的可能。}

  \note{\pgf{} 提供了一种很方便的写循环的方式：\cmd{foreach} \cmd{x} in \{1,...,10\}\{循环体\}}
