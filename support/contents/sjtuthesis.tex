  ...
  \begin{columns}[t]
    ...
    \begin{column}{0.25\textwidth}
      ...
      \only<2>{
        \begin{exampleblock}{}
          \begin{itemize}
            \item[\faBolt] \TeX{} 的可视性改良 \TeX{}\raise-0.25em\hbox{\footnotesize MACS} \link{http://texmacs.org/tmweb/home/welcome.en.html}
          \end{itemize}
        \end{exampleblock}
      }
    \end{column}
...
%   \end{codeblock}
  \frametitle{论文类型}
  % \framesubtitle{\textbackslash{}documentclass[type=bachelor]\{sjtuthesis\}}
  \begin{figure}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.6\textwidth}
      ...
      \only<2>{
        更多文档类选项，请查阅 \textsc{SJTU\TeX{}}(v1) 的开发文档 \link{https://github.com/sjtug/SJTUTeX/releases/download/v1.1.0/sjtuthesis.pdf}。
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.6\textwidth}
      在 \texttt{main.tex} 中引入 \texttt{setup.tex} 来导入主要的信息录入与宏包加载配置（见第 \ref{setup} 页）。
      其中 \cmd{sjtusetup} 中的 \pkg{info} 将会修改封面的信息设置（见第 \ref{covers} 页）。
