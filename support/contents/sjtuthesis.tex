...
|\phantom{}|\documentclass[type=master,review]{sjtuthesis}
      ...
      我是博士，先写着电子版不空页
      \begin{codeblock}[]{}
|\phantom{}|\documentclass[type=doctor,openany]{sjtuthesis}
      \end{codeblock}
    \end{column}
  \end{columns}
  \note{有 ChatGPT 那味了。}
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    ...
    \begin{column}{0.65\textwidth}
      ...
      \only<2>{
        \alert{(a,b)} 其中 \cmd{sjtusetup}（第 \ref{sjtusetup} 页）中的 \opt{info} 将会修改封面的信息设置。
      }
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.6\textwidth}
      \alert{(c)} \cmd{copyrightpage} 可以用于插入版权页。
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.6\textwidth}
      \alert{(d,e,f,g,h)} 前言从 \cmd{frontmatter} 处开始，页码设置为大写罗马数字，主要包含摘要和目录内容。
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.6\textwidth}
      \alert{(i,j,k)} 正文从 \cmd{mainmatter} 处开始，页码设置为正常数字，包含正文、参考文献、附录内容。
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.6\textwidth}
      \alert{(k,l,m,n)} 结尾从 \cmd{backmatter} 处开始，页码设置为正常数字，包含致谢等相关情况。
