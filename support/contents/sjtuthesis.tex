  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
      \begin{itemize}
        ...
        \item 2022 年 11 月模板改版后，吴伟健、李子龙、张驰等人使用 \LaTeX3 重构 2.0 版本
    ...
    \begin{column}{0.4\textwidth}
      ...
      \begin{block}{}
        \scriptsize
        上海交通大学 \hologo{XeLaTeX} 学位论文模板 | Shanghai Jiao Tong University \hologo{XeLaTeX} Thesis Template
      \end{block}
      \vspace{-8pt}
      \begin{alertblock}{}
        \scriptsize
  ...
  \begin{columns}[t]
    \begin{column}{0.25\textwidth}
      ...
      \only<2>{
        \begin{block}{}
          \begin{itemize}
            \item[\faBolt] R Markdown (Bookdown) 模板 \link{https://github.com/bubifengyun/SJTUThesis-Rmd}
    ...
    \begin{column}{0.25\textwidth}
      ...
      \only<2>{
        \begin{block}{}
          \begin{itemize}
            \item[\faBolt] 致力于像 \LaTeX 一样强大，但同时简单易用
          \end{itemize}
        \end{block}
      }
    \end{column}
...
\begin{frame}
  ...
  \alert{Git 克隆} 使用 Git 克隆 GitHub SJTUThesis 仓库 \link{https://github.com/sjtug/SJTUThesis} 或 SJTUG 镜像仓库。
  \begin{exampleblock}{\faGit*}
    \ttfamily\small
    git clone https://mirror.sjtu.edu.cn/git/SJTUThesis.git/
  \end{exampleblock}

  \alert{下载压缩包} 在 GitHub 仓库的 Release 页面 \link{https://github.com/sjtug/SJTUThesis/releases} 下载最新的 Source code (zip)。
  \medskip

  \alert{在线平台} 直接使用 Overleaf 链接 \link{https://www.overleaf.com/latex/templates/sjtuthesis-latex-thesis-template-for-shanghai-jiao-tong-university/mkdwbyjbtfgg?r=sdkbtJ4qGS8kDZQQ&rm=d&rs=b}。其他在线平台用户可以下载压缩包，上传至对应平台。
\end{frame}

\begin{frame}
  \frametitle{编译}

  \alert{在线平台} Overleaf 等在线平台在打开模板或上传压缩包后，选择 \hologo{XeLaTeX} 编译器即可编译。
  \medskip

  \alert{VS Code + \LaTeX Workshop} 在命令面板（Ctrl + Shift + P）搜索运行 Build with recipe 并选择 \texttt{latexmk (xelatex)} 即可编译。可在 VS Code 配置文件中设置 \texttt{latex-workshop.latex.recipe.default} 为 \texttt{lastUsed}，此后修改文件并保存时会沿用上次选择的 recipe。
  \medskip

  \alert{手动编译} 推荐使用 \pkg{latexmk} 编译\footnote{\hologo{MiKTeX} 用户需要手动安装 Perl 解释器 \link{https://www.perl.org/get.html} 才能使用 \pkg{latexmk}。}，在不能够利用自带的 \texttt{.latexmkrc} 配置文件的情况下，需要查清楚在对应的编辑器中如何使用 \hologo{XeLaTeX} + \hologo{biber} 编译\footnote{这种情况下，你可能需要查清楚如何全局安装该文档类，并刷新文件名数据库。} \link{https://github.com/sjtug/SJTUThesis/blob/master/README.md}。
...
\begin{frame}[label=compile]
  ...
  \begin{columns}
    ...
    \begin{column}{0.33\textwidth}
      ...
      \end{block}
      \begin{block}{\faComments{} Discussions}
        如果实在无法解决，可以前往 Discussions \link{https://github.com/sjtug/SJTUThesis/discussions} 提问交流。
      \end{block}
    \end{column}
  \end{columns}
  \note{两个进阶问题：
...
\begin{frame}[fragile]
  ...
  \begin{columns}
    ...
    \begin{column}{0.65\textwidth}
      ...
      \only<2>{
        \alert{\textbf{(a,b)}} \texttt{setup.tex} 文件中 \cmd{sjtusetup} 设置的 \opt{info} 将会修改封面信息。
      }
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \end{column}
    \begin{column}{0.6\textwidth}
      \SJTUThesis{} 定义了常用的数学环境（需要在 \texttt{setup.tex} 中引入 \pkg{ntheorem} 或者 \pkg{amsthm} 宏包）。
