...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.6\textwidth}
      ...
      \only<2>{
        更多文档类选项，请查阅 \textsc{SJTU\TeX{}}(v1) 的开发文档，老版本开发文档中有些接口可能已废弃 \link{https://github.com/sjtug/SJTUThesis/releases/download/1.0.0rc7/sjtuthesis.pdf}。
...
\end{codeblock}
\only<1>{
  引入 \texttt{setup.tex} 来导入主要的信息录入与宏包加载配置（见第 \ref{setup} 页）。
}
...
|\highlightline<2>|% \copyrightpage[scans/sample-copyright-old.pdf]
      ...
      \only<2>{
        \cmd{copyrightpage} 也接受一个可选参数，用于直接使用扫描件。\thesisissue{473}
      }
    \end{column}
  \end{columns}
\end{frame}
