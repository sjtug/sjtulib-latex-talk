...
\begin{frame}
  ...
  \alert{在线平台} Overleaf 在线平台在打开模板或上传压缩包后，选择 \hologo{XeLaTeX} 编译器即可编译。
  \medskip

  \alert{VS Code + \LaTeX Workshop} 在命令面板（Ctrl + Shift + P）搜索运行 Build with recipe 并选择 \texttt{latexmk (xelatex)} 即可编译。可在 VS Code 配置文件中设置 \texttt{"latex-workshop.latex.recipe.default": "lastUsed"}，此后修改文件并保存时会沿用上次选择的 recipe。
  \medskip
